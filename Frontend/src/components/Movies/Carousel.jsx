...
function Carousel() {
  ...
  useEffect(() => {
    if (status === "idle") {
      ...
      console.log(fetchMoviesLastest);
    }
  }, [dispatch, status]);
  return (
    <>
      ...
      >
        {status === "loading" ? (
          ...
          movies.movies.map((movie) => (
            <section
              key={movie._id}
              className="bg-fondo bg-cover flex flex-col justify-center items-center w-full min-h-[70vh] mb-10 "
            >
              <div className=" flex flex-col justify-center items-center p-1 w-full min-h-[70vh]">
