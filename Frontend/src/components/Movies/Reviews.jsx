  ...
  return (
    <>
      {review.length === 0 ? (
        ...
        <section className="flex flex-col justify-start items-center text-white w-full p-5 mt-5 border-r-2 border-b-2 border-opacity-20 border-solid border-black min-h-[30vh] sm:w-[45%] sm:border-r-4 sm:border-b-4 gap-3 md:text-xl xl:w-[30%]">
          <div className="w-full flex justify-between">
            <h2 className="font-Coda w-full">
              @{review.usuario.usuario}
            </h2>
            {data.isAdmin ||
              (data.id === review.usuario._id && (
                <ModalDeleteReview
                  id={review._id}
